...
{this->motor_id = motor_id;
  ...
  if(can_driver->begin(MCP_STDEXT, CAN_1000KBPS, MCP_16MHZ) == CAN_OK) {
    // if(can_driver->begin(MCP_STDEXT, CAN_10KBPS, MCP_8MHZ) == CAN_OK) {
    // 
...
void CanBus::writeCalibrationResult(uint8_t result)
{
 ...
 // can_driver->sendMsgBuf(0x10 + 2, 0, 2, data_res);
  //can_driver->sendMsgBuf(0x10 + 3, 0, 2, data_res);
  //can_driver->sendMsgBuf(0x10 + 4, 0, 2, data_res);
}
...
{   
    ...
    switch (cmd) {
      case CAN_CMD_POSITION: // 3 data bytes
        {//Serial.println("CAN_CMD_POSITION");
          if (len < 4) { break; } // pb
      ...
      case CAN_CMD_MODE: 
        ...
        Serial.println(control_mode1);
          Serial.print("Goal step : ");
         Serial.println(control_mode2);
